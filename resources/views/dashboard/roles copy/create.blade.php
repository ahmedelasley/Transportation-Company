    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                <div class="text-bold h5">إنشاء دور جديد</div>
            </div>
        </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        @foreach (\Spatie\Permission\Models\Permission::select('type', 'type_name')->groupBy('type', 'type_name')->get() as $data)
                            ...
                            <div class="my-2 h6 font-weight-bold d-block"><b><input type="checkbox" onClick="check_uncheck_checkbox{{ $data->type }}(this.checked);"> {{ $data->type_name }} </b></div>

                            <div class="row bg-light py-3">
                                
                                @foreach($permission->where('type', $data->type) as $value)
                                    <div class="col-xs-4 col-sm-4 col-md-3">
                                        <label>{{ Form::checkbox('permission[]', $value->id, false, array('class' => 'checkbox_'.$data->type)) }}
                                        {{ $value->name }}</label>
                                        <br/>
                                    </div>
                                @endforeach
                            ...
                            <script>
                                function check_uncheck_checkbox{{ $data->type }}(isChecked) {
                                    if(isChecked) {
