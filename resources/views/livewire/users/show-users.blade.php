                    ...
                    <div class="mb-3 mb-xl-0 z-3">
                        <div class="btn-group dropdown">
                            ...
                            <select class="form-control rounded" wire:model.live="sortBy">
                                ...
                                @forelse (\Schema::getColumnListing('users')  as $row)
                            ...
                            <table class="table table-striped table-striped-columns table-hover table-bordered border-primary table-sm text-center">
                                <thead class="bg-primary sticky-top" style="z-index: 0;">
                                    <tr>
                                        ...
                                        <th class="wd-10p"><h6><b>{{ __('Email') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Roles') }}</b></h6></th>
                                ...
                                <tbody class="table-group-divider tx-14 tx-bold">
                                    ...
                                    <tr>
                                        ...
                                        <td> {{ $value->email }} </td>
                                        <td>
                                            @forelse ($value->roles as $v )
                                                <span class="tag tag-primary tx-white">{{ $v->name }}</span>
                                            @empty
                                                <span class="tag tag-danger tx-white">{{ __('No Roles')}}</span>
                                            @endforelse
                                        </td>

                                        <td>
                                            <a class="btn btn-warning btn-sm" href="#" data-toggle="modal" data-target="#passwordModal" wire:click.prevent='getForm({{ $value->id }})'><i class="fa fa-lock"></i> </a>
                                            <a class="btn btn-primary btn-sm" href="#" data-toggle="modal" data-target="#assignRoleModal" wire:click.prevent='getForm({{ $value->id }})'><i class="fa fa-sliders-h"></i> </a>
                                        ...
                                        <tr>
                                            <td colspan="13">
                                                <div class="alert alert-info tx-20" role="alert">
                                                    ...
                                                    <span class="alert-inner--text"><strong> {{ __('alert.Alert') }} ! </strong> {{ __('alert.There is no data yet !', ['type' => __('alert.users')]) }}</span>
                                                </div>    
                                            </td>
                                        </tr>
                                    @endforelse
