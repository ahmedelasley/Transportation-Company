<div>
    ...
    @include('livewire.statuses.modal')
                    ...
                    <div class="mb-3 mb-xl-0 z-3">
                        <div class="btn-group dropdown">
                            ...
                            <select class="form-control rounded" wire:model.live="sortBy">
                                ...
                                @forelse (\Schema::getColumnListing('statuses')  as $row)
                ...
                <div class="card col-md-12">
                            ...
                            <table class="table table-striped table-striped-columns table-hover table-bordered border-primary table-sm text-center">
                                <thead class="bg-primary sticky-top" style="z-index: 0;">
                                    <tr>
                                        ...
                                        <th class="wd-15p"><h6><b>{{ __('Name') }}</b></h6></th>
                                        <th class="wd-15p"><h6><b>{{ __('Description') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Parent') }}</b></h6></th>
                                ...
                                <tbody class="table-group-divider tx-14 tx-bold">
                                    ...
                                    <tr>
                                        <th class="bg-gray-100" scope="row">{{ $statuses->firstItem()+$loop->index }}</th>

                                        <td> {{ $value->name }} </td>
                                        <td> {{ $value->description }} </td>
                                        <td>
                                             {!! $value->parent_id == NULL ? '( ' . $value->children->count() . ' ) Sub value' : '<span class="tag  tag-primary tx-white">' . $value->parent->name . '</span> ( ' . $value->children->count() . ' ) Sub value' !!}
                                        </td>
                                        <td>
                                        ...
                                        <tr>
                                            <td colspan="7">
                                                <div class="alert alert-info tx-20" role="alert">
                                                    ...
                                                    <span class="alert-inner--text"><strong> {{ __('alert.Alert') }} ! </strong> {{ __('alert.There is no data yet !', ['type' => __('alert.statuses')]) }}</span>
                                                </div>    
                                            </td>
                                        </tr>
                                    @endforelse
                    ...
                    <div class="card-footer">
                        <div class="d-flex flex-row justify-content-end">
                            {{-- <div class=""> --}}
                                <ul class="pagination">
                                    {{ $statuses->withQueryString()->onEachSide(5)->links() }} 
                                </ul>
                            {{-- </div> --}}
                        </div>
                    </div>
