                    ...
                    <div class="mb-3 mb-xl-0 z-3">
                        <div class="btn-group dropdown">
                            ...
                            <select class="form-control rounded" wire:model.live="sortBy">
                                ...
                                @forelse (\Schema::getColumnListing('categories')  as $row)
                        ...
                        <div class="table-responsive table-wrapper-scroll-y my-custom-scrollbar" style="height : calc(100vh - 350px)">
                            <table class="table table-striped table-striped-columns table-hover table-bordered border-primary table-sm text-center">
                                ...
                                <tbody class="table-group-divider tx-14 tx-bold">
                                    ...
                                    <tr>
                                        <th class="bg-gray-100" scope="row">{{ $categories->firstItem()+$loop->index }}</th>

                                        <td> {{ $value->name }} </td>
                                        <td> {{ $value->description }} </td>
                                        <td>
                                             {!! $value->parent_id == NULL ? '( ' . $value->children->count() . ' ) Sub Category' : '<span class="tag  tag-primary tx-white">' . $value->parent->name . '</span> ( ' . $value->children->count() . ' ) Sub Category' !!}
                                        </td>
                                        <td>
                                        ...
                                        <tr>
                                            <td colspan="7">
                                                <div class="alert alert-info tx-20" role="alert">
                                                    ...
                                                    <span class="alert-inner--text"><strong> {{ __('alert.Alert') }} ! </strong> {{ __('alert.There is no data yet !', ['type' => __('alert.Categories')]) }}</span>
                                                </div>    
                                            </td>
                                        </tr>
                                    @endforelse
