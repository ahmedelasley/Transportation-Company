        ...
        <div class="modal-content modal-content-demo">
            ...
            <div class="modal-body">
                <form id="form" >
                    ...
                    <div class="form-group">
                        <label class="main-content-label h6 font-weight-bold">{{ __('Parent') }}</label>
  
                        <div class="input-group" >
                            <select class="form-control h6 font-weight-bold" wire:model.live="form.parent_id" >
                                <option class="h6 font-weight-bold" label="{{ __('Choose Parent') }}" ></option>
                                <option class="h6 font-weight-bold" value="0" >{{ __('No Parent') }}</option>
                        ...
                        @error('form.parent_id')<span class="bg-danger tx-white d-block px-1 py-1">{{ $message }}</span>@enderror
                    </div>
                </form>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group" >
                            <select class="form-control h6 font-weight-bold" wire:model.live="form.parent_id" >
                                <option class="h6 font-weight-bold" label="{{ __('Choose Parent') }}" ></option>
                                <option class="h6 font-weight-bold" value="0" >{{ __('No Parent') }}</option>
                                @forelse ($categories->where('id', '!=', $form->ids ) as $value)
                                    <option class="h6 font-weight-bold" value="{{ $value->id }}">{{ $value->name }}</option>
                                @empty
                                @endforelse
