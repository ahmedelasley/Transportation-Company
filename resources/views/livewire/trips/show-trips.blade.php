                    ...
                    <div class="mb-3 mb-xl-0 z-3">
                        <div class="btn-group dropdown">
                            ...
                            <select class="form-control rounded" wire:model.live="sortBy">
                                ...
                                @forelse (\Schema::getColumnListing('trips')  as $row)
                ...
                <div class="card col-md-12">
                            ...
                            <table class="table table-striped table-striped-columns table-hover table-bordered border-primary table-sm text-center">
                                <thead class="bg-primary sticky-top" style="z-index: 0;">
                                    <tr>
                                        ...
                                        <th class="wd-10p"><h6><b>{{ __('Code') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Client / Company') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Date') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('From Area') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('To Area') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Vehicle') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Total') }}</b></h6></th>
                                        <th class="wd-10p"><h6><b>{{ __('Paid') }}</b></h6></th>
                                ...
                                <tbody class="table-group-divider tx-14 tx-bold">
                                    @forelse ($trips as $value)
                                    <tr>
                                        <th class="bg-gray-100" scope="row">{{ $trips->firstItem()+$loop->index }}</th>

                                        <td> <a class="" href="" data-toggle="modal" data-target="#showModal" wire:click.prevent='getForm({{ $value->id }})'> {{ $value->code }} <a class="" href="" data-toggle="modal" data-target="#showModal" wire:click.prevent='getForm({{ $value->id }})'></td>
                                        <td> {{ $value->client ? $value->client->name : $value->company->name }} </td>
                                        <td> {{ $value->date }} </td>
                                        <td> <span class="tag tag-primary tx-white">{{ $value->fromArea->name }} </span></td>
                                        <td> <span class="tag tag-success tx-white">{{ $value->toArea->name }} </span></td>
                                        <td> <span class="tag tag-warning tx-white">{{ $value->vehicle->name }} </span></td>
                                        <td> {{ number_format($value->service_cost + $value->wait_cost, SettingHelper::setting('price') ) }} </td>
                                        <td> {{ number_format($value->paid, SettingHelper::setting('price') ) }} </td>
                                        ...
                                        <tr>
                                            <td colspan="7">
                                                <div class="alert alert-info tx-20" role="alert">
                                                    ...
                                                    <span class="alert-inner--text"><strong> {{ __('alert.Alert') }} ! </strong> {{ __('alert.There is no data yet !', ['type' => __('alert.trips')]) }}</span>
                                                </div>    
                                            </td>
                                        </tr>
                                    @endforelse
                    ...
                    <div class="card-footer">
                        <div class="d-flex flex-row justify-content-end">
                            {{-- <div class=""> --}}
                                <ul class="pagination">
                                    {{ $trips->withQueryString()->onEachSide(5)->links() }} 
                                </ul>
                            {{-- </div> --}}
                        </div>
                    </div>
